...
基于此机制我们提出 manifold pipeline：Dynamics-aware CSDI imputation（含三处稳定性改善：非零门初始化、per-dimension centering、Bayesian 软锚定带噪观测）→ 延迟坐标回归（Lorenz63 用 SVGP；Lorenz96 高维下 SVGP AR rollout 架构失效，替换为 DeepEDM 的 softmax-attention-as-learned-kernel，§5.7.3）→ Lyapunov-empirical conformal 校准。Lorenz63 S3 下 pipeline VPT 达 Panda 的 **2.2×**、Parrot 的 **7.1×**；21 个 (场景, horizon) cell 上 PI 偏离 nominal 0.90 ≤ **2%**。跨系统外部有效性在 Lorenz96 N=20 上兑现：**S5/S6 极端 harshness 下 CSDI + DeepEDM 是唯一存活的方法（VPT = 0.74 / 0.49 Λ），Panda / Parrot / Persist 全部归零**——即 Theorem 2(a) 预言的基础模型 tokenizer OOD 阈值 $s > s^*$ 之外 dynamics-aware pipeline 的优势。代码、CSDI checkpoint 与数据开源。
...
**贡献 3（实证）.** Lorenz63 S3 场景下 pipeline 达 **Panda 的 2.2×、Parrot 的 7.1×**；S4 扩大到 **Panda 的 9.4×**（CSDI 升级后，详见 §5.3）。Panda 实测 −85% 与 Theorem 2(a) 下界 −44% + OOD −41% 的分解在数量级上闭环。21 个 (场景, horizon) cell 上 PI 偏离 nominal 0.90 ≤ 2%（比 Split CP **3.2× 更准**）。**跨系统外部有效性 + 架构升级**：Lorenz96 N=20 × 5 seeds × 6 methods × 7 scenarios = 210 runs，完成 (i) 两 bug 定位并修复（M1 归一化、M3 inducing-points）、(ii) M3 架构从 SVGP 换成 DeepEDM（ICML 2025，§5.7.3）、(iii) 最终表：Panda / Parrot / Persist 在 S5-S6 全部 VPT = 0，**唯独 CSDI + DeepEDM 保 VPT = 0.74 / 0.49 Λ**——Theorem 2(a) OOD 阈值 $s > s^*$ 后基础模型预训数据失效、dynamics-aware manifold pipeline 存活的直接实证。代码、12 张 figures、CSDI checkpoint + DeepEDM / FNO M3 backbone 均开源。
...
3. **极端 harshness 下唯一存活的方法**：S5/S6（95-97% sparsity, 1.2-1.5σ noise）上，Panda / Parrot / Persist 全部 VPT = 0，唯独 **CSDI + DeepEDM 保持 VPT = 0.74 / 0.49 Λ**。Std 较大（±0.81 / ±0.64）意味着部分 seed 也归零，但均值显著 > 0 —— 正是 Theorem 2(a) 预言的 "$s > s^*$ 后基础模型 tokenizer OOD、而 dynamics-aware pipeline 依靠 $d_{KY}$-约束的延迟流形仍有可用信息" 的实证兑现。Panda 在 S0-S4 凭借 72M 参数 + billions-of-tokens 预训碾压我们 2-4×，但**一旦越过 tokenizer OOD 阈值（~S5），预训练数据全部失效，而我们的 manifold-pipeline 继续工作**。
...
**Scope.** 我们主要测了 Lorenz63（低维经典混沌，$d_{KY} \approx 2.06$）；Lorenz96 N=20 上：（a）SVGP 训练 scaling 随 $d_{KY}$ 而非 ambient $N$；（b）§5.7 用 5 seeds × 7 scenarios × 6 methods 的 PT eval 完成相变的跨系统外部验证，显示相变**跨系统普适**但 tipping 位置随 $\lambda_1 / d_{KY}$ 推后；（c）§5.7.3 诊断并修复了两个 bug（M1 CSDI 归一化、M3 SVGP m_inducing），且发现 SVGP 在 100-D 延迟特征上的 autoregressive rollout 是架构性瓶颈（α 从 $h=0$ 的 0.77 衰到 $h=20$ 的 0.27），因此把 M3 换成 **DeepEDM**（softmax-attention-as-learned-kernel on delay tokens）；修完后 **CSDI + DeepEDM 在 S5/S6 上仍保 VPT = 0.74 / 0.49 Λ**，是所有方法里唯一越过 Panda tokenizer OOD 阈值仍工作的——Theorem 2(a) 的正面实证。把更广系统扫描（Kuramoto-Sivashinsky、dysts benchmark [Gilpin23]、Mackey-Glass）以及真实数据 case study（EEG、气候 reanalysis、临床时序）仍是未来工作。
...
**训练预算不对称（对基线有利）.** 我们 CSDI M1 在 Lorenz63 上训练 512K 独立 IC windows（197M samples），L96 CSDI M1（2026-04-23 新训）在 Lorenz96 N=20 上 1M windows（2.56B samples），单系统单 dataset。Panda-72M [Wang25] 在 **billions of time-series tokens** 上预训，覆盖 dysts 20+ 混沌系统。基线训练数据量比我们大 1-3 个数量级；Panda 72M 参数也 ~57× 大于我们的 126 万。**我们的优势不在训练数据量，而在方法-场景匹配**：(i) M1 训练显式采样 $s \in U(0.2, 0.9) \times \sigma/\sigma_\text{attr} \in U(0, 1.5)$，是 sparse+noisy 测试场景的 in-distribution 设计；(ii) M2-M4 全套模块在延迟流形 $\mathcal{M}_\tau$ 上工作，收缩率由 $d_{KY}$ 而非 ambient $D$ 主导（§4 Thm 2(b)）；(iii) Panda / Chronos tokenizer 在 $s > 0.5$ linearly-interpolated context 上触发 OOD 跃变（§5.6 iii JS 3.1× 跃变），基线训练数据再多也不覆盖这一人工伪迹。用一个"基线训练数据 ≫ 我们"的设置中击败基线，恰恰说明 OOD 机制的理论预言正确。
...
基于此机制，我们构造的 manifold pipeline（CSDI imputation + 延迟流形 kernel regression + Lyapunov-empirical conformal）在 Lorenz63 S3 达 Panda 的 **2.2×**、Parrot 的 **7.1×**；21 个 (场景, horizon) cell 上 PI 偏离 nominal 90% ≤ 2%，显著紧于 Split CP。CSDI 需要三处稳定性改善（非零门初始化、per-dim centering、Bayesian 软锚定）才能在混沌轨迹上稳定训练；第三处改善的价值随 $\sigma^2$ quadratic 放大（S2 +53% / S4 +110% / S6 10× VPT），是 Theorem 2 $\sigma$-channel OOD 机制的直接实证。跨系统验证在 Lorenz96 N=20 × 5 seeds × 6 methods × 7 scenarios 上独立复现相变并完成 M3 架构升级（SVGP → DeepEDM，§5.7.3）：**极端 harshness S5/S6 下 CSDI + DeepEDM 是唯一非零方法（VPT = 0.74 / 0.49 Λ），Panda / Parrot / Persist 全部归零**——正是 Theorem 2(a) 预言的基础模型 tokenizer OOD 阈值外 dynamics-aware pipeline 的优势兑现。
