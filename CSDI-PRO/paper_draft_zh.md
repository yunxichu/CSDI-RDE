...
**重要限制声明（2026-04-24 诊断+修复+定位）.** 我们在 L96 上分别训练了新的 **L96-专用 CSDI M1 ckpt**（5 GPU × val-loss 早停 patience=20，channels ∈ {128, 192}、layers ∈ {8, 12}、variant ∈ {full, no_noise}）并做下游 L96 PT 评测与诊断可视化，分离出**两个独立问题**：

**问题 1 — M1 inference bug（已修）.** `csdi_impute_adapter` 未将 L96 的 `attractor_std=3.639` 透传到 `model.impute(...)`，推理时用 L63 默认值 8.51 做归一化，scale 错 **2.3×**，导致 CSDI 输出被压缩到 0.43× 真实幅度。修复 + ckpt cfg patch 后：L96 S3 上 CSDI 补值 RMSE = 1.42 vs linear 1.63，**CSDI 反超 linear 13%**（见 Fig L96-impute）—— CSDI M1 在 L96 上工作正常。

**问题 2 — M3 SVGP autoregressive rollout 架构失效（未修，是真正的 limitation）.** 即使 M1 修好、补值质量正常，**full pipeline 在 L96 N=20 上 S0 (clean) 下仍然快速塌陷为常数输出**（见 Fig L96-collapse：ours_csdi / ours_ark / ours_ark_L17 m_inducing=512 三个变体在 clean data 下都塌陷）。根因：
1. **Autoregressive 误差指数爆炸**：每一步 SVGP posterior sample 带微小误差,128 步后 rollout 累积偏离 attractor;
2. **Delay embedding 丢失 Ring topology**：L96 ring coupling $\dot x_i = (x_{i+1} - x_{i-2})x_{i-1} - x_i + F$ 在我们的 "per-dim delay features" 展平成 100-D 向量时,i±1 邻居耦合信息被丢;
3. **GP prior mean=0 吸引**：rollout 离训练流形远后,SVGP 回到 prior mean (0),在 20-D coupled attractor 上吸入极强。

**本节主要 claim 收窄为：相变现象跨系统普适（Panda, Parrot 在 L96 独立展现 phase transition）**。Our pipeline 的 M1 (CSDI) 在 L96 上补值**已经 validated 工作正常（比 linear 好 13%）**；但 M3 (SVGP-on-delay-coords + autoregressive rollout) 的架构对 L96 N=20 coupled system 不 scale，需要重新设计 — 候选方向包括 Koopman spectral (EDMD) 直接算子估计、Graph NN 保留 Ring topology、或基于 Parrot-style attractor 1-NN retrieval — 是明确的 **future work**。

见 [Fig L96-PT](experiments/week1/figures/pt_l96_N20_phase_transition.png)、[Fig L96-impute](experiments/week1/figures/l96_csdi_imputation_S3_seed0_fixed_nonoise_c192_best.png)（CSDI 修复后补值点贴近 truth）、和 [Fig L96-collapse](experiments/week1/figures/l96_forecast_S3_seed0_FIXED_nonoise_c192_best_S3.png)（修复后 full pipeline 仍塌陷 → M3 问题独立）；数据 `experiments/week1/results/pt_l96_l96_N20_v1.json` + `pt_l96_l96_N20_v1_seeds34.json`。

## 6. 讨论与限制

**Scope.** 我们主要测了 Lorenz63（低维经典混沌，$d_{KY} \approx 2.06$），并在 Lorenz96 上（a）确认 SVGP 训练 scaling 随 $d_{KY}$ 而非 ambient $N$；（b）§5.7 新增 Lorenz96 N=20 × 5 seeds × 7 scenarios 的 phase-transition 验证，显示相变现象**跨系统普适**但 tipping point 随 $\lambda_1 / d_{KY}$ 推后一格；（c）诊断发现**我们当前的 SVGP-on-delay-coords + autoregressive rollout pipeline 在 L96 N=20 上架构失效**（即使 clean data、retrained CSDI M1、或调大 $L_\text{embed}$/m_inducing 也不救），根因是 ring topology 丢失 + autoregressive 误差爆炸 + GP prior 吸入（见 §5.7 limitation）。L96 上击败 Panda/Parrot 需要 **M3 重新设计**（Koopman spectral / Graph NN / attractor-retrieval），是明确的 future work。把更广系统扫描（Kuramoto-Sivashinsky、dysts benchmark [Gilpin23]、Mackey-Glass）以及真实数据 case study（EEG、气候 reanalysis、临床时序）也是未来工作。
