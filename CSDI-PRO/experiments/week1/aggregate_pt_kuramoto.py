"""Aggregate Kuramoto PT eval results + markdown table + transition plot."""
...
def main(tag: str = "kuramoto_5seed"):
    path = RESULTS / f"pt_kuramoto_{tag}.json"
    if not path.exists():
        print(f"[warn] {path} missing"); return
    doc = json.loads(path.read_text())
    merged = doc.get("summary", {})
    print("\n### Table — Kuramoto VPT@1.0 (mean ± std)\n")
    ...
    ax.set_title(f"Kuramoto N=10 K=1.5 phase-transition ({tag})")
    ax.legend(loc="upper right", fontsize=9, framealpha=0.9)
    ax.grid(alpha=0.3); ax.set_ylim(0, None)
    plt.tight_layout()
    out = FIGS / f"pt_kuramoto_{tag}_phase_transition.png"
...
if __name__ == "__main__":
    import sys
    tag = sys.argv[1] if len(sys.argv) > 1 else "kuramoto_5seed"
    main(tag)
