"""FNO sanity test: parallel to svgp_l63_vs_l96_sanity.py / deepedm_l63_vs_l96_sanity.py.

Reports 1-step + autoregressive rollout α/RMSE on L63 and L96 under the same
truth-context regime (n_ctx=512, pred_len=128, taus=[4,3,2,1]).
"""
...
def run_system(name: str, traj_fn, taus: np.ndarray, n_ctx: int = 512,
               pred_len: int = 128, n_epochs: int = 400, seed: int = 0,
               width: int = 64, n_layers: int = 3, modes: int = 3):
    ...
    print(f"  [1-step] pred={mu1[0][:3]} ... truth={future[0][:3]} ... rmse={err1:.3f}  α={alpha1:.4f}")
...
if __name__ == "__main__":
    ...
    for r, (name, ctx, fut, pred) in enumerate([
        ...
        for c, d in enumerate(rep_dims):
            ...
            ax.plot(pred[:, d], "r-", linewidth=1, label="FNO rollout", alpha=0.8)
            ax.set_title(f"{name} dim {d}", fontsize=10)
            ax.grid(alpha=0.3)
            if c == 0:
                ax.legend(fontsize=9)

    plt.suptitle("FNO-on-truth sanity: L63 vs L96 forecast", fontsize=12)
    plt.tight_layout()
    out = Path(__file__).resolve().parent / "figures" / "fno_l63_vs_l96_sanity.png"
