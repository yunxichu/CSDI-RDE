"""Lorenz63 trajectory figure for §5.7 (companion to plot_l96_trajectory_final.py).

L63 is D=3 (x, y, z), much simpler than L96's 20-D coupled ring. Shows
(a) on L63 the old SVGP-based pipeline already works; (b) swapping M3 to
DeepEDM does NOT regress low-D performance; (c) Panda still wins clean
clean S0 but collapses at S5/S6 same as L96.

Output: CSDI-PRO/pictures/l63_trajectory_final.png
"""
...
from experiments.week1.lorenz63_utils import (
    ...
    integrate_lorenz63, LORENZ63_LYAP, LORENZ63_ATTRACTOR_STD,
)
...
OUT = REPO / "pictures" / "l63_trajectory_final.png"
OUT.parent.mkdir(parents=True, exist_ok=True)

DT, LYAP, ATTR_STD = 0.025, LORENZ63_LYAP, LORENZ63_ATTRACTOR_STD
N_CTX, PRED_LEN = 512, 128
CKPT = REPO / "experiments/week2_modules/ckpts/dyn_csdi_full_v6_center_ep20.pt"
...
def run_all():
    ...
    # L63 default attractor_std is 8.51 — no override needed, but set explicitly for clarity
    set_csdi_attractor_std(LORENZ63_ATTRACTOR_STD)

    traj = integrate_lorenz63(N_CTX + PRED_LEN, dt=DT, spinup=2000, seed=SEED)
    ...
    for sc in SCENARIOS:
        ...
        results[sc.name] = dict(
            fut=fut_true,
            csdi_deepedm=pred_csdi_deepedm,
            csdi_svgp=pred_csdi_svgp,
            ark_deepedm=pred_ark_deepedm,
            panda=pred_panda,
...
def plot(results):
    ...
    for r, sc in enumerate(SCENARIOS):
        ...
        for c, dim in enumerate(DIMS):
            ...
            if d["panda"] is not None:
                ax.plot(t, d["panda"][:, dim], color="C1", linewidth=1.1,
                        alpha=0.9, label="Panda-72M")
            ax.plot(t, d["csdi_svgp"][:, dim], color="C5", linewidth=1.1,
                    linestyle=":", alpha=0.85, label="Ours (CSDI + SVGP, legacy)")
            ax.plot(t, d["ark_deepedm"][:, dim], color="C0", linewidth=1.1,
                    linestyle="--", alpha=0.85, label="Ours (AR-K + DeepEDM)")
            ax.plot(t, d["csdi_deepedm"][:, dim], color="C3", linewidth=1.6,
    ...
    fig.suptitle(
        f"Lorenz63 forecast trajectories — seed {SEED}, n_ctx={N_CTX}, pred_len={PRED_LEN}  "
        f"(λ={LYAP:.3f}, σ_attr={ATTR_STD:.2f})\n"
        "L63 is D=3 (x/y/z); S5-S6 with 90-95% sparsity still leaves enough "
        "signal for all Ours variants to be on-attractor, Panda flattens.",
        fontsize=11,
    )
    plt.tight_layout()
