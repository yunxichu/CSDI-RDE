"""Rössler Panda jitter/residual control at SP65 (and SP82) transition anchors.

Mirror of `panda_jitter_control_l96.py` on the weak-chaos system Rössler.
Question: does the L96 finding — "iid jitter recovers most of mean VPT but
CSDI keeps the tail" — replicate on a system with very different λ_1 and
attractor scale?
"""
...
ROSSLER_CKPT = REPO / "experiments" / "week2_modules" / "ckpts" / "dyn_csdi_rossler_full_vales_best.pt"
...
def _plot(summary: dict[str, Any], out_png: Path) -> None:
    fig, axes = plt.subplots(1, len(summary), figsize=(6 * len(summary), 4.2),
                              squeeze=False)
    colors = {
        "linear": "C1",
...
def _write_md(summary: dict[str, Any], contrasts: dict[str, Any], out_md: Path) -> None:
    lines = ["# Rössler Panda Jitter Control", ""]
...
def main() -> None:
    ...
    ap.add_argument("--tag", default="rossler_sp65_sp82_5seed")
    args = ap.parse_args()
    ...
    attr_std = float(ROSSLER_ATTRACTOR_STD)
    set_csdi_checkpoint(str(ROSSLER_CKPT))
    set_csdi_attractor_std(attr_std)
    print(f"[rossler-jitter] CSDI ckpt={ROSSLER_CKPT}")
    print(f"[rossler-jitter] attr_std={attr_std:.4f} lyap={ROSSLER_LYAP}")
    ...
    for sc in SCENARIOS:
        ...
        for i in range(args.n_seeds):
            ...
            obs_res = make_corrupted_observations(
                ...
                dt=args.dt, lyap=ROSSLER_LYAP, patch_length=16,
            )
            ...
            for cell in CELLS:
                ...
                vpt03 = valid_prediction_time(future_true, mean, dt=args.dt,
                                               lyap=ROSSLER_LYAP, threshold=0.3,
                                               attractor_std=attr_std)
                vpt05 = valid_prediction_time(future_true, mean, dt=args.dt,
                                               lyap=ROSSLER_LYAP, threshold=0.5,
                                               attractor_std=attr_std)
                vpt10 = valid_prediction_time(future_true, mean, dt=args.dt,
                                               lyap=ROSSLER_LYAP, threshold=1.0,
                                               attractor_std=attr_std)
                rec = {
                    "scenario": sc["name"], "seed": int(seed), "cell": cell,
                    "sparsity": float(sc["sparsity"]),
                    "noise_std_frac": float(sc["noise_std_frac"]),
                ...
                records.append(rec)
                print(f"  seed={seed} {cell:22s} keep={obs_res.metadata['keep_frac']:.3f} "
    ...
    out_json.write_text(json.dumps({
        "config": vars(args) | {"csdi_ckpt": str(ROSSLER_CKPT)},
