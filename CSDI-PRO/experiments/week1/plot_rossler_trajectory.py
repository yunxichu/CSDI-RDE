"""Rössler trajectory figure for §5.10 — weak-chaos clean-regime showcase.

Rössler is 3D (x, y, z). λ_1 ≈ 0.07 → very long Lyapunov horizon.
4 scenarios (S0, S3, S5, S6) × 3 dims = 12 panels.
"""
...
from systems.rossler import (
    integrate_rossler, ROSSLER_ATTRACTOR_STD, ROSSLER_LYAP, ROSSLER_DT,
)
...
OUT = REPO / "pictures" / "rossler_trajectory_final.png"
OUT.parent.mkdir(parents=True, exist_ok=True)

DT, LYAP, ATTR_STD = ROSSLER_DT, ROSSLER_LYAP, ROSSLER_ATTRACTOR_STD
N_CTX, PRED_LEN = 512, 1024
CKPT = REPO / "experiments/week2_modules/ckpts/dyn_csdi_rossler_full_vales_best.pt"
...
def run_all():
    ...
    traj = integrate_rossler(N_CTX + PRED_LEN, dt=DT, spinup=2000, seed=SEED)
...
def plot(results):
    ...
    for r, sc in enumerate(SCENARIOS):
        ...
        for c, dim in enumerate(DIMS):
            ...
            ax.plot(t, d["svgp"][:, dim], color="C5", linewidth=1.1,
                    linestyle=":", alpha=0.85, label="Ours (CSDI + SVGP)")
            ax.plot(t, d["deepedm"][:, dim], color="C3", linewidth=1.5,
    ...
    fig.suptitle(
        f"Rössler forecast — seed {SEED}, n_ctx={N_CTX}, pred_len={PRED_LEN}  "
        f"(λ={LYAP:.3f}, σ_attr={ATTR_STD:.2f})\n"
        "Weak chaos (λ=0.07): long Lyapunov horizon, pred_len ≈ 7 Λ.",
        fontsize=11,
    )
    plt.tight_layout()
