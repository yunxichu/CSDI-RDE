"""Chua double-scroll phase-transition evaluation (§5.12)."""
...
from systems.chua import integrate_chua, CHUA_LYAP, CHUA_ATTRACTOR_STD, CHUA_DT
...
def run_pilot(n_seeds, n_ctx, pred_len, dt, methods, seed_offset=0):
    attr_std = CHUA_ATTRACTOR_STD
    lyap = CHUA_LYAP
    print(f"[pilot-chua] dt={dt} attr_std={attr_std:.3f} lyap={lyap} "
          f"n_seeds={n_seeds} n_ctx={n_ctx} pred_len={pred_len} methods={methods}", flush=True)
    records = []
...
def main():
    ...
    ap.add_argument("--tag", default="chua_v1")
