"""Kuramoto N=10 K=1.5 trajectory figure. State = stacked (cos,sin) → D=20."""
...
from systems.kuramoto import (
    integrate_kuramoto, KURAMOTO_ATTRACTOR_STD, KURAMOTO_LYAP, KURAMOTO_DT, KURAMOTO_N,
)
...
OUT = REPO / "pictures" / "kuramoto_trajectory_final.png"
OUT.parent.mkdir(parents=True, exist_ok=True)

DT, LYAP, ATTR_STD = KURAMOTO_DT, KURAMOTO_LYAP, KURAMOTO_ATTRACTOR_STD
N_CTX, PRED_LEN = 512, 512
CKPT = REPO / "experiments/week2_modules/ckpts/dyn_csdi_kuramoto_full_vales_best.pt"
...
SEED = 0
# Pick representative (cos θ_i) dims at i = 0, N/2, N-1 (skip sin half)
DIMS = [0, KURAMOTO_N, 2 * KURAMOTO_N - 2]  # cos θ_0, cos θ_5, cos θ_9
DIM_NAMES = [f"cos θ_{0}", f"cos θ_{KURAMOTO_N // 2}", f"cos θ_{KURAMOTO_N - 1}"]
...
def plot(results):
    ...
    for r, sc in enumerate(SCENARIOS):
        ...
        for c, dim in enumerate(DIMS):
            ...
            if r == 0:
                ax.set_title(DIM_NAMES[c], fontsize=10)
            if r == nrows - 1:
                ax.set_xlabel("horizon (Lyapunov times Λ)", fontsize=9)
            ax.grid(alpha=0.3)
            if r == 0 and c == ncols - 1:
                ax.legend(loc="upper right", fontsize=7, framealpha=0.9)
    fig.suptitle(f"Kuramoto N={KURAMOTO_N} K=1.5 forecast — seed {SEED}, n_ctx={N_CTX}, "
