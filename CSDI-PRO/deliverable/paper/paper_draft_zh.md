...
在 Figure-1 协议下，我们对比 clean、linear-fill、CSDI-fill 与 SAITS-pretrained-fill 四种 context。§4.4 替代 imputer 比较显示 CSDI 与 corpus-pretrained SAITS 都救援 Panda；本节问的是它们是否共享同一 Panda-内部机制——即**通用** corpus-pretrained 结构化 imputation 是否减少 token 距离-到-clean，还是这种减少 CSDI-specific。

**Raw-patch（L63 SP65, 10 seeds, v2 协议）.**
...
**Panda-内部 token 空间（L63, 5 seeds, v2 协议, 4-cell 配对距离-到-clean，跨 token 平均 L2）.**
...
| Stage | SP65 linear/CSDI | SP65 linear/SAITS | SP82 linear/CSDI | SP82 linear/SAITS | **SP82 SAITS/CSDI** |
|:--|:-:|:-:|:-:|:-:|:-:|
...
两个 regime-specific 事实凸显。

**入口带（SP65）.** CSDI 与 SAITS-pretrained 都把 token 推向 clean，幅度不同：CSDI 在每个 Panda 阶段达到相对 linear ~12× 减少，SAITS 达到 ~2× 减少。"**Corpus-pretrained 结构化 imputation 减少 Panda-token 距离-到-clean**"机制对**两个** imputer 都成立——不是 CSDI-specific。CSDI 的更大减少跟踪其在该 cell 的小 paired VPT 优势（CSDI − SAITS = +0.41 Λ, §4.4）：减少越多 → 救援越强。

**底部带（SP82）.** 两个 imputer 仍然相对 linear 减少 token 距离，但 SAITS-pretrained 与 CSDI 之间在最终 pooled stage 的 rank order 反转：SAITS 产生**比 CSDI 更小**的 token 距离-到-clean（SAITS/CSDI 比 0.57–0.95）。然而在 §4.4 同一 cell 的预测上，CSDI 与 SAITS-pretrained 在统计上不可分辨（paired CSDI − SAITS = +0.06 Λ，CI [−0.31, +0.59]）。仅看**最终 pooled** 距离，"更小 token 距离的 imputer 不是 VPT 更好的 imputer"看似矛盾。下面的逐层探测解决了这个悖论：不是 encoder-vs-decoder，而是**哪一层 encoder 是 rescue-relevant** 的问题。

Raw 度量印证边界：SP82 处 local stdev 与 mid-frequency power 偏 CSDI（3.54× 与 5.19×），而 lag-1 自相关偏 linear（0.62×）。

**逐层 encoder 探测（12-layer 扫描，L63, 5 seeds, v2 协议）.** Panda-72M 是 encoder-only（其 head 是从 pooled encoder state 的线性投影），所以我们在每个 PandaLayer 输出上注册 forward hook，逐层度量配对 L2-到-clean：
...
**SP65 每一层都保持 CSDI 比 SAITS 更接近 clean 4–6×** —— encoder geometry 在每一层都 informative，并跟踪 CSDI 严格正 paired VPT 优势。**SP82 的逐层图景是非单调的**：SAITS 在层 0–5 与 9–12 上更近（saits/csdi 0.64–0.94），但在**中间 encoder 层 6–8 处两者收敛到 tie（saits/csdi 1.00–1.06）**。中间 encoder 带是 Panda 内部表征中**唯一**SAITS 与 CSDI 看起来 equivalently close to clean 的子区域——而这恰好匹配 §4.4 的预测 tie（CSDI − SAITS = +0.06 Λ，CI [−0.31, +0.59]）。

这是直接测量证据，表明 §4.4 底部带救援**在 Panda 中间 encoder 处饱和**：到层 7，两个 corpus-pretrained imputer 已经产生在距离-到-clean 上统计不可分辨的内部表征，后续层的任何区分都不会传递到可预测性，因为 head 的线性投影看到的是 pooled（≈ averaged）state，平滑掉了中间 encoder 的收敛。上面距离条形表里的 pooled-only reading 因此是**晚层 artefact**，不是 VPT 的相关预测器。

机制主张因此读为：

> **入口带：corpus-pretrained 结构化 imputation 在 encoder 每一层都均匀地减少 raw / Panda-token OOD 距离-到-clean，减少幅度跟踪救援强度。底部带：两个 corpus-pretrained imputer 的表征在 Panda 中间 encoder（层 6–8）处收敛到 tie；这就是 §4.4 预测 tie 的实现处。最终 pooled 距离是晚层 artefact，不是救援相关几何。**
...
- **基础模型可解释性.** §4.2 的逐层 encoder 探测通过直接测量解决了原"decoder-side latent dynamics"假设：Panda-72M 是 encoder-only（其 head 是从 pooled encoder state 的线性投影），底部带救援**在中间 encoder 层 6–8 处饱和**，那里 SAITS-pretrained 与 CSDI 表征收敛到距离-到-clean 上的 tie（saits/csdi 1.00–1.06）。最终 pooled 距离是晚层 artefact，不是相关几何。剩下开放的是**为何**中间 encoder 是饱和带——即 Panda 内部哪种特定计算由层 6–8 执行，使得两个 early-layer 距离差异很大的 corpus-pretrained imputer 到达等价的 mid-layer state。这是后续可解释性问题，不是 §1 / §3 / §4 主张的 blocker。
...
| 8 | Panda embedding OOD 诊断（加 SAITS arm，P3.B1）| L63 | SP65, SP82 | clean / linear / **SAITS-pretrained** / CSDI；阶段 patch / embed / encoder / pooled | 5 | `panda_embedding_ood_l63_sp65_sp82_dt025_v2protocol_patched_with_saits_5seed.json`（当前 §4.2 来源）；旧 3-cell 文件 `..._5seed.json` 保留用于 Figure 2 PCA 散点 | `..._with_saits_5seed.md` 与 `_bars.png`；PCA 散点用旧 3-cell 文件 |
...
| 17 | **逐层 encoder 探测（P3.B2）** | L63 | SP65, SP82 | clean / linear / SAITS-pretrained / CSDI × {12 个 PandaLayer 输出 + post-embedder} | 5 | `panda_per_layer_probe_l63_sp65_sp82_per_layer_5seed.json` | §4.2 中间 encoder 收敛发现；通过直接测量关闭"decoder-side 假设" |
