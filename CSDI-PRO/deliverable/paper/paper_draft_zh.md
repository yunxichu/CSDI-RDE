# 稀疏观测 OOD 相变：混沌时间序列基础模型的失败机制与重构
...
预训练时间序列预测器在稀疏含噪的混沌观测下会突然失效，但失效源不只是有效样本数下降。我们发现，常见的“先线性插值、再送入基础模型”会制造非物理 patch，把 tokenized forecaster 推到训练分布之外。Lorenz63 上，Panda-72M 与 Parrot 在 S3（$s=0.6$, $\sigma/\sigma_\text{attr}=0.5$）前后损失超过 85% VPT；Lorenz96 N=20 上，Panda 在线性填补协议下到 S5/S6 归零。一个 90-run 的 $(s,\sigma)$ grid 和 patch-curvature OOD 测量显示，稀疏插值主要伤害 ambient/tokenized forecaster，而延迟流形重构更受噪声通道限制。

关键实验是 imputer × forecaster 的隔离矩阵：{linear, Kalman, CSDI} × {Panda, delay-manifold DeepEDM}。在 Lorenz96 N=20 上，把 Panda 的输入从 linear-fill 换成 CSDI-fill，使 S4 VPT 从 0.52 提升到 3.60 Lyapunov times，$\Pr(\mathrm{VPT}>0.5)$ 从 60% 提升到 100%；paired bootstrap 给出 +3.07 Λ 增益，95% CI [+0.57,+6.45]。同样的 imputer 替换也提升 DeepEDM 的 S3/S4 表现。因此本文的干净结论不是“基础模型天生不适合混沌”，也不是“延迟坐标是唯一幸存者”，而是：**稀疏观测的预处理会成为 OOD 失败通道；corruption-aware imputation 是第一缓解杠杆，delay-manifold forecasting 是第二条动力学结构化通道**。MG 与 Chua 的 negative results 作为 scope condition 报告，而不是隐藏失败。

---

## 1. 引言

**稀疏观测先是预处理问题，然后才是预测问题。** 混沌预测 benchmark 通常给模型密集干净的 context window，而真实观测会掉点、带噪、抖动。下游系统通常先填补缺失值再预测；这个 fill step 可能制造一个从未由物理系统生成过的对象。对混沌轨迹而言，穿过弯曲吸引子的长线性片段不是温和伪迹，而是模型直接看到的 OOD patch。

**相变现象.** 我们扫 7 个 harshness 场景（S0-S6，稀疏率 $0\%\to95\%$，噪声 $\sigma/\sigma_\text{attr}:0\to1.5$），发现 Panda / Parrot / Chronos 不是平滑退化，而是在 transition band 中突然崩塌。Lorenz63 上 Panda S0→S3 损失 85% VPT，Parrot 损失 92%；Lorenz96 N=20 上 Panda 在线性填补协议下到 S5/S6 归零。VPT 在相变带 seed 方差很大，因此本文同时报告 mean±std、paired bootstrap CI 和 survival probability $\Pr(\mathrm{VPT}>0)$ / $\Pr(\mathrm{VPT}>0.5)$。

**机制隔离.** 我们用 2×3 矩阵回答一个更尖锐的问题：Panda 失败是因为 ambient/tokenized forecaster 天生不适合 sparse-noisy chaos，还是因为填补后的 context 本身已经 OOD？L96 N=20 的结果显示，`CSDI → Panda` 在 S3/S4 显著强于 `linear → Panda`，并且是 S4 最强 cell。这推翻了“delay-manifold 是唯一幸存通道”的过强说法。更准确的说法是：**corruption-aware imputation 移除了预处理诱发的 OOD 通道；delay-manifold forecasting 是受益于同一 imputation 质量的动力学结构化伴随通道。**

**方法定位.** 因此，方法不再写成 `delaymask/random delay embedding` 新模块，而是写成 corruption-aware reconstruction pipeline：CSDI imputation with noisy-observation anchoring + delay-manifold forecasting（Lorenz63 用 SVGP，Lorenz96 用 DeepEDM）+ Lyapunov-empirical conformal coverage。CSDI 三个修复不作为“修 bug”售卖，而解释为混沌轨迹在 noisy observation model 下需要的 diffusion anchoring 条件。Conformal coverage 与 random delay embedding 降为副贡献或附录。

### 1.1 主要贡献

**贡献 1（现象）.** 我们系统记录预训练混沌时间序列预测器在 sparse-noisy observation 下的相变式退化，并用 survival probability 和 bootstrap interval 治理 VPT 高方差，而不只报告 mean-only 表。

**贡献 2（机制隔离）.** 通过 imputer × forecaster 矩阵隔离 fill-before-forecast 通道。L96 N=20 显示 CSDI-filled context 可在 S3/S4 救回 Panda，说明主要 failure source 往往是 preprocessing-induced OOD patch geometry，而非 ambient/tokenized forecaster 的先天无能。

**贡献 3（缓解与边界）.** 我们构建 corruption-aware reconstruction pipeline：CSDI imputation + delay-manifold forecasting + Lyapunov-empirical conformal。理论主张收窄为：对 interpolation-filled sparse observations 下的 tokenized pretrained forecaster，OOD patch geometry 会引入 excess-risk 项；delay-coordinate estimators 在 smooth attractor 假设下避开这一特定 OOD 通道。Mackey-Glass 与 Chua 作为 scope boundary 报告。

**论文结构.** §2 相关工作；§3 方法；§4 理论；§5 实验（phase transition、$(s,\sigma)$ grid、机制隔离矩阵、L96 跨系统验证）；§6 讨论；§7 结论。Conformal coverage、random delay embedding、delay-mask variants 放入附录级材料。
...
### 3.3 模块 M3 — 延迟流形上的 kernel regression

给定延迟坐标数据集 $\{(\mathbf{X}_\tau(t), x_{t+h})\}$，我们学一个把延迟 token 映射到下一状态的回归器。低维系统（Lorenz63, $D=3$, 15-D 延迟特征）用 **Matérn-5/2 稀疏变分 GP（SVGP）** + inducing points（默认 128，按 $\max(128, 5 \times \text{feat\_dim})$ 自适应），Lorenz96 $N \in \{10, 20, 40\}$ 训练 $25 \to 42 \to 92$s——**近 $N$-线性**，exact GPR 在 $N=40$ OOM。高维系统（Lorenz96 $N=20$, 100-D 延迟特征）下 SVGP 的 AR rollout 出现 α 逐步退化（α@h=20 = 0.27，§5.7.2），故 M3 支持替换为 **DeepEDM**（Majeedi et al., ICML 2025）：把 $L{+}1$ 个延迟 token 喂入 3 层 softmax self-attention，$\text{softmax}(QK^\top/\sqrt{d})V$ 等价于 data-dependent kernel regression on the delay manifold——保留 Takens / EDM 叙事，但 attention 复杂度 $O(nL)$、learned kernel off-manifold 不 prior-attract 到 0。收敛率理论分析仍走 Kaplan-Yorke 维 $d_{KY}$（Lorenz96 $\approx 0.4N$）主导而非 ambient $N$（§4 Thm 2(b) + 附录 E）。backbone 通过 `full_pipeline_rollout.py` 的 `backbone ∈ {svgp, deepedm, fno}` flag 切换。
...
### 5.7 跨系统验证：Lorenz96 N=20 相变 + M3 架构升级（2026-04-23/24）

**动机.** §5.2 在 Lorenz63 ($D=3$, $d_{KY} \approx 2.06$) 上建立相变现象；Theorem 2 预言此现象对任意满足 §4.1 设定的光滑遍历混沌系统都应出现。为检验外部有效性，我们在 **Lorenz96 N=20 F=8**（$d_{KY} \approx 8$，$\lambda_1 \approx 1.68$）上复制 §5.2 设置。

**Setup.** Lorenz96 N=20 × 7 harshness scenarios × 6 methods × **5 seeds** = 210 runs。$dt=0.05$，$n_\text{ctx}=512$, $\text{pred}\_\text{len}=128$，attr_std = 3.639（50k 步积分经验）。两种 M1 变体：**AR-Kalman M1**（§5.2 对齐）和 **CSDI M1**（L96-专用 ckpt，$c=192$, val-loss 早停 patience=20，1M 独立 IC windows）。Panda-72M / Parrot 使用 linear-interp filled context。

#### 5.7.1 两个被定位并修复的 bug

**Bug 1 — M1 CSDI inference 归一化（已修）.** `csdi_impute_adapter` 未将 L96 的 `attractor_std=3.639` 透传到 `model.impute(...)`，推理时用 L63 默认值 8.51，scale 错 **2.3×**（CSDI 输出被压缩到 0.43× 真实幅度）。修复 + ckpt cfg 补丁后：L96 S3 上 CSDI 补值 RMSE = 1.42 vs linear 1.63，**CSDI 反超 linear 13%**（见 Fig L96-impute）。

**Bug 2 — M3 SVGP inducing-points 不足（已修）.** 在 L96 的 100 维延迟特征空间里，原 `m_inducing=128` 使 Matérn-5/2 核 $\exp(-\|x-x'\|^2/2\ell^2)$ 在 $\|x-x'\| \sim \sqrt{D} \gg \ell=1$ 时几乎归零，GP 输出塌陷为训练目标均值。启发式修复：`m_inducing = max(128, 5 \times \text{feat\_dim})`，$\text{L63}$ 不变、$\text{L96}$ 自动升到 500。修复后 L96 的 1-step scale ratio $\alpha = \langle\hat y, y\rangle/\|y\|^2$ 从 0.375 升到 0.766（Fig M3-comparison）。

#### 5.7.2 M3 架构性限制：SVGP 在高维 delay manifold 上的 AR rollout 仍塌陷

两 bug 修好后，**SVGP 在 1-step 正常、但多步自回归 rollout 里仍逐步退化**。3-seed 诊断（Fig `m3_backbone_comparison_l96.png`, clean truth context，n_ctx=512）给出 $\alpha$ vs horizon $h$：

| $h$ | SVGP (m=500, 修复后) | DeepEDM | FNO |
|:-:|:-:|:-:|:-:|
...
**诊断归因**：Matérn-5/2 SVGP 在 100-D 延迟特征上，误差 geometric cumulative——每一步小偏离经 GP posterior smoothing 乘以 $<1$ 的 Lipschitz 后被拉回 prior mean 0，但 rollout 过程中真实 attractor 仍以 $e^{\lambda_1 h}$ 发散，两者差距指数放大。这是 **SVGP-on-delay-manifold 架构的固有瓶颈**，不是 inducing-points 能救的。

#### 5.7.3 M3 替换为 DeepEDM（ICML 2025 LETS Forecast）

我们把 M3 从 SVGP 替换为 **DeepEDM**（Majeedi et al., ICML 2025, arXiv:2506.06454）：把延迟坐标 $[x(t), x(t-\tau_1), \dots, x(t-\tau_L)]$ 展平成 $L{+}1$ 个 token，用 3 层 multi-head softmax attention 把"kernel regression on delay manifold"参数化为可学的 attention 核。数学上 $\text{softmax}(QK^\top/\sqrt{d})V$ 就是 data-dependent kernel regression，保留 §3.3 的 Takens/EDM 叙事；但 attention 复杂度 $O(nL)$ 而非 GP 的 $O(n^3)$，同时 learned kernel 能 off-manifold 泛化，不会 prior-attract 到 0。我们也试了 **1-D FNO**（Li 2021 / Liu 2024）作备选，在 Fourier 域沿 lag 轴做 spectral conv，结果列在对比中。

实现：`models/deep_edm.py`、`models/fno_delay.py`，均匹配 SVGP 的 `.fit(X, Y)/.predict(X)` 契约，`full_pipeline_rollout.py` 通过 `backbone ∈ {svgp, deepedm, fno}` flag 切换。代码 + smoke test + 3-backbone α-vs-horizon 对比图均开源。

#### 5.7.4 最终 PT eval 结果（5 seeds × 7 scenarios × 6 methods = 210 runs）
...
SVGP legacy 行用 3 seeds（每次 SVGP 拟合 87s，总耗时约 25 min）；数字与 5-seed 主表趋势一致，主要用于量化 M3 swap 的增益：S0 上 DeepEDM 给 AR-K pipeline **+125%** 改善（0.48 → 1.08），S3 上 **+164%**（0.14 → 0.37）。SVGP 在 S4-S6 完全归零，DeepEDM 把 S4-S5 推到 0.2-0.3 Λ，CSDI+DeepEDM 把 S4-S6 推到 0.5-0.7 Λ。
...
1. **相变现象跨系统普适**（原 §5.7 主 claim 保留）：Panda S0→S5 跌 100%、Parrot S0→S3 跌 87%、所有基线在 S5/S6 全部归零——L96 的相变 tipping 从 S3 开始且在 S5 完全崩塌，与 Theorem 2 的 $n_\text{eff}$ 临界 + tokenizer KL 跃变 (§5.6 iii) 机制闭环。

2. **M3 swap 把 L63 成功故事搬到 L96**：AR-K + DeepEDM 在 S0 已达 1.08 Λ（超过 Parrot 2.1×），与 §5.2 Lorenz63 S0 量级一致；CSDI + DeepEDM 进一步把 S3-S6 的 VPT 推升一个数量级（S3: 0.67 vs Parrot 0.07，**9.6×**；S4: 0.71 vs 0.12，**5.9×**）。

3. **CSDI 是关键 OOD 缓解杠杆**：原始 PT 表中，Panda / Parrot / Persist 都拿 linear-filled context，并在 S5/S6 归零；CSDI + DeepEDM 仍有非零 mean VPT。下一节的 isolation matrix 进一步细化解释：当 Panda 拿到 CSDI-filled context 时，它在 S3/S4 可被显著救回，有时 S5 也非零。因此结论不是“foundation models 必然失败”，而是 **linear fill 制造 OOD patch；corruption-aware imputation 移除了很大一部分伪迹**。

见 [Fig §5.7 main](experiments/week1/figures/pt_l96_m3alt_phase_transition.png)、[Fig M3-backbone](experiments/week1/figures/m3_backbone_comparison_l96.png)、[Fig L96-impute](experiments/week1/figures/l96_csdi_imputation_S3_seed0_fixed_nonoise_c192_best.png)（CSDI M1 工作正常）；数据 `experiments/week1/results/pt_l96_m3alt_merged.json`。

### 5.8 机制隔离：imputer × forecaster 矩阵

为区分“补值器救了模型”与“延迟流形 forecaster 救了模型”，我们跑了 2×3 矩阵：imputer ∈ {linear, Kalman, CSDI}，forecaster ∈ {Panda-72M, delay-token DeepEDM}，聚焦 S2-S5 transition band。

**L96 N=20，VPT@1.0 mean ± std，n=5 seeds；括号为 $\Pr(\mathrm{VPT}>0.5)$。**
...
Paired bootstrap 显示，对 Panda 而言，CSDI 替换 linear fill 在 S3 带来 +1.01 Λ（95% CI [+0.12,+2.50]），S4 带来 +3.07 Λ（[+0.57,+6.45]）；对 DeepEDM，S3/S4 分别是 +0.47 Λ（[+0.05,+0.84]）与 +0.64 Λ（[+0.20,+1.09]）。

同一矩阵已复制到 L63、L96 N=10 与 Rössler。L63/L96 上 CSDI 对 Panda 的 rescue 最清楚；Rössler 上绝对 VPT 较低，但 CSDI 对 DeepEDM 在 S3/S4/S5 有更稳定的正增益。这个矩阵是本文收窄主张的原因：**delay coordinates 不是唯一幸存者；预处理诱发的 OOD 通道才是更直接的 failure mechanism**。

数据与图表：`experiments/week1/results/pt_*_iso_*_5seed.json`；`experiments/week1/figures/iso_*_5seed.md`；`experiments/week1/figures/iso_*_5seed_{heatmap,bars}.png`。

## 6. 讨论与限制

**Scope.** 我们主要测了 Lorenz63（低维经典混沌，$d_{KY} \approx 2.06$）和 Lorenz96 N=20；L96 上完成了 5 seeds × 7 scenarios × 6 methods 的 PT eval，并用 isolation matrix 定位到 preprocessing-induced OOD 通道。SVGP 在 100-D 延迟特征上的 autoregressive rollout 是架构性瓶颈，因此 M3 换成 DeepEDM。最新 isolation 结果改变了 scope statement：CSDI + DeepEDM 不是“唯一可能幸存者”；一旦预处理被修好，Panda 也可在 S3/S4 被救回。更安全的结论是 **corruption-aware reconstruction 是共享关键成分，可同时帮助 Panda 和 delay-manifold forecaster**。更广系统扫描（Kuramoto-Sivashinsky、dysts benchmark [Gilpin23]）与真实数据 case study（EEG、气候 reanalysis、临床时序）仍是未来工作。
...
**基础模型公平性.** 原 baseline 协议让 Panda / Chronos 接收线性插值后的观测，而不是 raw NaN context。这是常规且表面上有利的选择，但也正是 OOD 通道的触发条件：高稀疏率下线性插值会产生非物理直线段。isolation matrix 因此加入更诊断性的 `CSDI → Panda` baseline；它在 S3/S4 显著救回 Panda，说明 paper 最强 claim 不是“我们的 forecaster 总是打败 Panda”，而是“观测模型和预处理分布决定 Panda 看到的是物理轨迹还是 OOD 伪迹”。

**训练预算不对称（对基线有利）.** 我们的 CSDI M1 是单系统训练；Panda-72M [Wang25] 在 billions of time-series tokens 上预训，覆盖 dysts 20+ 混沌系统。isolation result 给出一个实用折中：可以保留基础模型，但替换 fragile interpolation。换言之，优势不必来自“forecaster 更大/更小”，而来自 observation model 与 preprocessing distribution 的匹配。

---

## 7. 结论

我们给出 sparse-noisy chaotic observations 下基础时间序列模型相变的机制解释。收窄后的 claim 很具体：对 interpolation-filled sparse observations 下的 tokenized pretrained forecaster，非物理 filled patches 会引入 OOD excess-risk 通道。90-run $(s,\sigma)$ grid 与 patch-curvature OOD 测量支持这个 preprocessing channel；新的 isolation matrix 进一步定位干预点：把 fragile interpolation 换成 CSDI 可在 S3/S4 救回 Panda。

基于此机制，我们的 mitigation stack 组合 CSDI imputation、delay-manifold forecasting 与 Lyapunov-empirical conformal calibration。Lorenz63 上它改善 harsh-regime survival；Lorenz96 N=20 上，isolation matrix 显示 CSDI 同时提升 Panda 与 DeepEDM 的 S3/S4 表现。最清楚的 takeaway 是：**稀疏含噪混沌不只是减少样本数；它会改变 forecaster 看到的 token/patch distribution，而 corruption-aware preprocessing 可以移除这一失败模式。** Delay-coordinate forecaster 仍重要，因为它提供 smooth attractor 假设下的 dynamics-structured path，但实验排除了“它是唯一可行通道”的强说法。
