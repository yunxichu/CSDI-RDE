...
**底部带（SP82）.** 两个 imputer 仍然相对 linear 减少 token 距离，但 SAITS-pretrained 与 CSDI 之间的 rank order **翻转**：SAITS 在每个阶段产生**比 CSDI 更小**的 token 距离-到-clean（SAITS/CSDI 比 0.57–0.95）。但在 §4.4 同一 cell 的预测上，CSDI 与 SAITS-pretrained 在统计上不可分辨（paired CSDI − SAITS = +0.06 Λ，CI [−0.31, +0.59]）。**Token 距离因此是 informative 的——两个 corpus-pretrained imputer 都相对 linear 减少它、都救援 Panda——但它在底部带不能 order SAITS-vs-CSDI**，更小 token 距离的 imputer（SAITS）并不是 VPT 更好的 imputer。这直接证明底部带 corpus-pretrained imputer 之间残留的可预测性差异**不是**由 encoder-side 距离捕获的，支持 §6.4 假设：底部带附近 decoder-side latent dynamics 起作用（其插桩留作 camera-ready）。

Raw 度量印证这一边界：SP82 处 local stdev 与 mid-frequency power 偏 CSDI（3.54× 与 5.19×），而 lag-1 自相关偏 linear（0.62×）。机制主张因此读为：

> **入口带：corpus-pretrained 结构化 imputation 减少 raw / Panda-token OOD 距离-到-clean，且减少幅度跟踪救援强度。底部带：两个 corpus-pretrained imputer 都减少 token 距离到足够救援的程度，救援达到饱和；哪个 VPT 上更优不再被 encoder-side token 距离 order。**
