...
**作者.** （待定）  **目标会议.** NeurIPS / ICLR 2026  **状态.** 锁定故事中文稿，2026-05-01。

> 本稿与 `paper_draft_en.md` 同步对应，按 `STORY_LOCK_2026-04-28.md`（含 2026-04-30 协议对齐 override）和 `PAPER_STATUS_2026-05-01.md` 锁定的故事写作。
> 旧版四模块流水线 + 通用 tokenizer-OOD 框架已归档到 `paper_draft_zh_archive_2026-04-30.md`。
>
> 摘要与 §1 中**必须保留**的三个限定语：
> (i) **稀疏观测**前沿（不是任何 corruption）；
> (ii) **transition band 之内**（不是普适改进）；
> (iii) **结构化残差与同等量级 iid 噪声并不可互换**，尤其在 tail survival 上。
>
> 所有数字来自 `experiments/{week1,week2_modules}/results/` 下的 JSON；图引用对应
> `deliverable/figures_*/` 与 `experiments/week1/figures/`。
...
主要数字（Figure-1 v2 patched 协议）：

- **L63 SP65**：CSDI 喂入 Panda 把 mean VPT 从 1.22 提升到 2.87 Lyapunov times（paired-bootstrap CI [+1.39, +1.91]），$\Pr(\mathrm{VPT} > 1.0)$ 从 70% 升到 100%；iid jitter 与 shuffled CSDI 残差均不能复制此增益。
- **L96 N=20 SP82**：Panda median VPT 从 0.50 → 1.05；$\Pr(\mathrm{VPT} > 0.5)$ 从 60% → 100%；DeepEDM paired CSDI−linear 增益 +0.43 [+0.29, +0.57]。L96 mean 在 n=10 仍然高方差，因此正文用 median 与 survival、不用 mean。
- 在纯噪声轴每一档 σ 上，CSDI 都对 Panda 中性或略有伤害——直接验证"间隙补全杠杆，不是 denoiser"。
...
regime 限定是必需的。我们在稀疏观测前沿之内观察到三个相关 regime：

1. **入口带 CSDI regime.**（L63 SP65，Figure-1 协议下）`CSDI → Panda` mean VPT 2.87 vs `linear → Panda` 1.22，paired-bootstrap 增益 +1.65 Λ，CI [+1.39, +1.91]；raw-patch 与 Panda-token 距离同时朝 clean 移动。
2. **通用正则化 regime.**（L96 N=20 SP65）iid jitter、shuffled CSDI 残差、CSDI 三者都把 mean VPT 推过 linear；但 CSDI 给出最佳 median 与最强 tail survival（$\Pr(\mathrm{VPT}>1.0)$：CSDI 60% vs jitter / shuffled 40% vs linear 20%）。
3. **底部带 CSDI regime.**（transition band 深处）通用噪声不再够，distance-to-clean 度量不再有判别力。L63 SP82：CSDI 比 linear 提升 +1.09 Λ，CI [+0.65, +1.61]；jitter 与 shuffled 均不跨 0。L96 N=20 SP82（n=10）：Panda median 0.50 → 1.05，$\Pr(\mathrm{VPT}>0.5)$ 60% → 100%；mean 仍高方差。

我们额外明示一个**纯噪声 non-rescue regime**：在 $s=0$、$\sigma>0$ 时 CSDI 不改善 Panda，在更大 σ 处略有伤害。CSDI 因此是稀疏间隙补全杠杆，不是通用密集噪声 denoiser。
...
**Regime-aware 机制.** 我们证明入口带救援与 raw-patch 和 Panda-token OOD 减少同步，而底部带的 survival 不能仅由 distance-to-clean 解释。

**Regime 分类.** Jitter 与 shuffled-residual 控制把入口带 CSDI 救援、高维通用正则化、底部带 CSDI 生存、纯噪声 non-rescue 区分开来，避免"CSDI 普适有效"的过强主张。

**动力学结构化伴随.** 我们保留延迟流形 forecasting 作为穿越前沿的**第二条**动力学路线，并明示其 scope condition。
...
### 4.4 替代 imputer reviewer-defense

一个 reviewer 自然会问：是结构化 imputation 本身就够，还是 CSDI 特定的动力学感知扩散残差是必要的？作为附录 sanity check，我们在 L63 SP65 跑 SAITS 与 BRITS 的**逐实例**拟合（单条 512 步轨迹，无预训语料）；两者均坍塌到 mean VPT 0.15 / 0.16，远低于 `linear → Panda` (1.29) 且远低于 `CSDI → Panda` (2.90，撞 ceiling)。这**对 SAITS / BRITS 不公**——它们正常会在大语料上预训，与 CSDI 的训练 pipeline 匹配。附录-only 结论因此很窄：**逐实例训练的通用结构化 imputer 在高缺失下不是有效替代**。预训练替代 imputer 比较（在 CSDI 用过的同一混沌语料上训 SAITS / Glocal-IB）是自然的后续工作；其设计与决策规则在 §6.4 与附录 C。
...
正文主图待测的 forecaster 是 Panda-72M [Wang25]。§4 的隔离矩阵覆盖 $\{$linear, AR-Kalman, CSDI$\} \times \{$Panda, DeepEDM$\}$。§4.4 reviewer-defense 表在最强的 CSDI-decisive cell 加入 SAITS 与 BRITS；两者都不在混沌语料上预训，因此对它们不公——我们在 §6.4 保留这一 caveat。
...
我们**不**主张预训练混沌 forecaster 本质上坏掉了——救援结果显示它们在 corruption-aware 填补之下高度可恢复。我们**不**主张 CSDI 是唯一能 work 的 imputer——只是我们测试过的唯一能稳定地穿越 transition band 的 intervention；§4.4 替代 imputer 实验给出当前边界，预训练 SAITS / Glocal-IB 比较推迟到附录 C。我们**不**主张机制已被完全刻画——distance-to-clean 解释入口带，但不解释底部带。我们**不**主张普适跨所有 foundation forecaster——Panda-72M 是 headline，Chronos / context-parroting 仅作为 side baseline。
...
- **语料不对称.** 我们的 CSDI 在每系统单独预训（每系统约 50 万独立 IC 窗口）。§4.4 的 SAITS 与 BRITS 是在单条 512 步轨迹上**逐实例**训练的——没有预训语料。预训练 SAITS / BRITS / Glocal-IB（在 CSDI 用过的同一混沌语料上）比较是自然后续。
- **单一 forecaster headline.** Panda 是唯一在 v2 corruption grid 下完整评测的 foundation forecaster。把前沿曲线在 TimesFM / Chronos / Lag-Llama 上复制将扩展外部有效性。
...
## 附录 C：预训练替代 imputer 计划（待跑）

见 `deliverable/EXPERIMENT_C_PLAN.md`。reviewer-defense 运行在 CSDI 用过的同一混沌语料上训练 SAITS（与如可实现的 Glocal-IB），然后在 L63 SP82 与 L96 N=20 SP82 评测。决策规则：

- 替代 imputer 复现可比救援 → 把主张收窄为"结构化 / 全局感知 imputation 是杠杆；CSDI 是其中一个强实例"；
- 替代 imputer 失败 → 强化"动力学感知重构是必要的"。

两种结果都保留 §3 前沿与 §6 scope condition。
