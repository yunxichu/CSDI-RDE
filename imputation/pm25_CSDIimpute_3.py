...
- 输出 3 个文件：
    ...
    2) history_missing_positions.csv     # 历史段缺失位置（长表：datetime/column/original/imputed）
    3) history_imputed.csv               # 历史段补值后的完整矩阵（后续预测统一使用它）
...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'rde_gpr', 'csdi'))
...
def main():
    ...
    # 1) 原始数据（history ground）
    hist_full_csv = os.path.join(out_dir, "history_full.csv")
    hist_full.to_csv(hist_full_csv)

    # 3) 补值后的完整历史段
    hist_imputed_csv = os.path.join(out_dir, "history_imputed.csv")
    df_hist_imputed.to_csv(hist_imputed_csv)

    # 2) 缺失位置输出：把缺失位置抽成“长表”
    miss_mask = hist_miss.isna()  # True 表示原 missing 文件里该位置缺失
    if miss_mask.values.any():
        miss_rows, miss_cols = np.where(miss_mask.values)

        miss_df = pd.DataFrame({
            "datetime": hist_miss.index.values[miss_rows],
            "column": np.array(hist_miss.columns)[miss_cols],
            "original_value": hist_miss.values[miss_rows, miss_cols],  # 基本都是 NaN
            "imputed_value": df_hist_imputed.values[miss_rows, miss_cols],
        })
        miss_df = miss_df.sort_values(["datetime", "column"]).reset_index(drop=True)
    else:
        miss_df = pd.DataFrame(columns=["datetime", "column", "original_value", "imputed_value"])

    hist_missing_pos_csv = os.path.join(out_dir, "history_missing_positions.csv")
    miss_df.to_csv(hist_missing_pos_csv, index=False)

    # meta
    meta_out = {
        **meta,
        "history_full_csv": "history_full.csv",
        ...
        "missing_rows_exported": int(len(miss_df)),
    }
    with open(os.path.join(out_dir, "impute_meta.json"), "w") as f:
        json.dump(meta_out, f, indent=4, ensure_ascii=False)

    print("\n完成！输出目录：", out_dir)
    print("history_full.csv =", hist_full_csv)
    print("history_missing_positions.csv =", hist_missing_pos_csv)
    print("history_imputed.csv =", hist_imputed_csv)
