...
运行示例：
python imputation/eeg_CSDIimpute.py \
  --run_folder ./save/eeg_random_ratio0.5_fold0_20260330_200026 \
  --missing_ratio 0.5 \
  --missing_mode random \
  --device cuda:0 \
  --seed 42 \
...
def load_csdi_eeg(model_path: str, config_json_path: str, device: str, target_dim: int):
    with open(config_json_path, "r") as f:
        full_cfg = json.load(f)
    config = full_cfg.get("model_config", full_cfg.get("config", full_cfg))

    model = CSDI_EEG(config, device, target_dim=target_dim).to(device)
...
def visualize_imputation_results(missing_positions, out_dir, n_features=64, features_to_show=None):
    ...
    if feature_rmse:
        ...
        ax.set_ylabel('RMSE')
        ax.set_title('Imputation RMSE per Feature (Channel)')
        ax.grid(True, alpha=0.3, axis='y')

        plt.tight_layout()
        plt.savefig(os.path.join(out_dir, 'imputation_rmse_per_feature.png'), dpi=150, bbox_inches='tight')
        plt.close()
        print(f"保存各通道RMSE图: {os.path.join(out_dir, 'imputation_rmse_per_feature.png')}")
    ...
    for i, feat in enumerate(features_to_show):
        ...
        if len(feat_df) > 0:
            ...
            if len(feat_df) > 0:
                rmse = np.sqrt(np.mean((feat_df['original_value'] - feat_df['imputed_value'])**2))
                ax.text(0.02, 0.95, f'Channel {feat}: RMSE={rmse:.2f}, n={len(feat_df)}',
                       transform=ax.transAxes, fontsize=10, verticalalignment='top',
                       bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

        ax.set_xlabel('Time Step')
        ax.set_ylabel('Value')
        ax.set_title(f'Channel {feat} - Original vs Imputed')
...
def main():
    parser = argparse.ArgumentParser(description="EEG Imputation (CSDI_EEG)")
    ...
    parser.add_argument("--visualize_features", type=str, default="0,1,2,3,4,5", help="可视化的通道索引")
    args = parser.parse_args()

    set_global_seed(args.seed)

    print("=" * 80)
    print("EEG 补值（CSDI_EEG）")
    print("=" * 80)
    ...
    model, _, _ = load_csdi_eeg(model_path, config_json, args.device, target_dim=n_features)

    now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = args.out_dir or f"./save/eeg_imputed_{args.missing_mode}_ratio{args.missing_ratio}_seed{args.seed}_{now}/"
    ensure_dir(out_dir)
    ...
    gt_values = dataset.gt_values
    gt_masks = dataset.gt_masks
    observed_data = dataset.observed_data
    observed_masks = dataset.observed_masks

    print(f"\nCSDI imputing {total_len} timesteps x {n_features} channels...")

    chunk_len = 100
    stride = 100

    for start in tqdm(range(0, total_len, stride), desc="CSDI imputing"):
        end = min(start + chunk_len, total_len)

        obs_data = observed_data[start:end]
        obs_mask = observed_masks[start:end]
        gt_mask = gt_masks[start:end]
        gt_data = gt_values[start:end]

        imputed = csdi_impute_eeg_batch(
            model=model,
    ...
    np.save(os.path.join(out_dir, "eeg_imputed.npy"), hist_imputed_merged)
    np.save(os.path.join(out_dir, "eeg_full.npy"), hist_full_merged)

    if all_missing_positions:
        miss_df = pd.DataFrame(all_missing_positions)
        miss_df.to_csv(os.path.join(out_dir, "eeg_missing_positions.csv"), index=False)
    else:
        pd.DataFrame(columns=["time_step", "feature", "original_value", "imputed_value"]).to_csv(
            os.path.join(out_dir, "eeg_missing_positions.csv"), index=False
        )

    meta_out = {
        **meta,
        "eeg_imputed_npy": "eeg_imputed.npy",
        "eeg_full_npy": "eeg_full.npy",
        "eeg_missing_positions_csv": "eeg_missing_positions.csv",
        "missing_count": len(all_missing_positions),
    }
    with open(os.path.join(out_dir, "impute_meta.json"), "w") as f:
        json.dump(meta_out, f, indent=4, ensure_ascii=False)

    print("\n完成！输出目录：", out_dir)
    print("eeg_imputed.npy =", os.path.join(out_dir, "eeg_imputed.npy"))
    print("eeg_full.npy =", os.path.join(out_dir, "eeg_full.npy"))
    print("eeg_missing_positions.csv =", os.path.join(out_dir, "eeg_missing_positions.csv"))
    ...
    if not args.skip_visualization and all_missing_positions:
        ...
        visualize_imputation_results(all_missing_positions, out_dir, n_features=n_features, features_to_show=features_to_show)

        visualize_imputation_heatmap(hist_imputed, hist_full, gt_masks, out_dir,
                                     n_timesteps_show=args.visualize_timesteps)

        print("\n生成的可视化文件:")
        print("  - imputation_error_distribution.png (误差分布)")
        print("  - imputation_rmse_per_feature.png (各通道RMSE)")
