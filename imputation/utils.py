...
def train(model, config, train_loader, valid_loader=None, valid_epoch_interval=50, foldername=""):
    ...
    for epoch_no in range(config["epochs"]):
        ...
        with tqdm(train_loader, mininterval=5.0, maxinterval=5.0) as it:
            for batch_no, train_batch in enumerate(it, start=1):
                ...
                optimizer.step()
                it.set_postfix(
                    ordered_dict={"avg_epoch_loss": avg_loss / batch_no, "epoch": epoch_no + 1},
                    refresh=False
                )
            lr_scheduler.step()
...
def evaluate(model, test_loader, nsample=100, scaler=1, foldername=""):
    with torch.no_grad():
        ...
        crps = calc_quantile_CRPS(all_target, all_generated_samples, all_evalpoint, 0, scaler)
        
        rmse = np.sqrt(mse_total / evalpoints_total)
        mae = mae_total / evalpoints_total
        
        print(f"RMSE: {rmse:.4f}, MAE: {mae:.4f}, CRPS: {crps:.4f}")
