...
运行示例：
  python pm25_gruodebayes_forecast.py \
    ...
    --window_size 48 --delta_t 0.1 --time_scale 0.1 --solver euler \
    ...
    for epoch in tqdm(range(epochs), desc="  训练", disable=not verbose):
        ...
        for (idx,) in loader:
            ...
            try:
                ...
                inc_loss = nn.functional.mse_loss(pred - x_last, y_batch - x_last)
...
def main():
    ...
    parser.add_argument("--time_scale", type=float, default=1.0,
                        help="时间缩放因子。如果=1.0，时间点为 0,1,2,...,W-1")
    ...
    print("\n[训练阶段]")
    t0 = time.time()
    pred_step = float(args.steps_ahead)
