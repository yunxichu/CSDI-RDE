...
def main():
    ...
    model_path = os.path.join(base_dir, 'CSDI-GPR_Lorenz_fullpy', 'save', 'model.pth')      # 训练好的CSDI模型路径
    config_path = os.path.join(config_dir, 'lorenz.yaml') # 模型配置文件路径
    ...
    # 3. Plot prediction 1 (original) with uncertainty - 绘制基于原始数据的预测结果和不确定性
    ...
    # 预测值
    plt.plot(pred_time_indices, result1, 'rx', markersize=8, label='Prediction 1 (Original)', linewidth=2)

    # 不确定性区域 (±2σ)
    plt.fill_between(pred_time_indices,
                     result1 - 2*std1,
                     result1 + 2*std1,
                     alpha=0.3, color='red', label='Prediction 1 ±2σ uncertainty')

    # 4. Plot prediction 2 (imputed) with uncertainty - 绘制基于补全数据的预测结果和不确定性
    if result2 is not None and std2 is not None:
        plt.plot(pred_time_indices, result2, 'm+', markersize=10, label='Prediction 2 (Imputed)', linewidth=2)

        # 不确定性区域 (±2σ)
        plt.fill_between(pred_time_indices,
                         result2 - 2*std2,
                         result2 + 2*std2,
                         alpha=0.3, color='magenta', label='Prediction 2 ±2σ uncertainty')
    ...
    plt.plot(pred_time_indices, result1, 'r--', label='Prediction 1 (Original)', linewidth=2)
    plt.fill_between(pred_time_indices, result1 - 2*std1, result1 + 2*std1, alpha=0.3, color='red')
    
    if result2 is not None and std2 is not None:
        plt.plot(pred_time_indices, result2, 'm--', label='Prediction 2 (Imputed)', linewidth=2)
    ...
    # 下子图：不确定性对比
    plt.subplot(2, 1, 2)
    plt.plot(pred_time_indices, 2*std1, 'r-', label='Prediction 1 Uncertainty (±2σ)', linewidth=2)
    plt.plot(pred_time_indices, np.abs(error1), 'r:', label='|Prediction 1 Error|', alpha=0.7)
    
    if result2 is not None and std2 is not None:
        plt.plot(pred_time_indices, 2*std2, 'm-', label='Prediction 2 Uncertainty (±2σ)', linewidth=2)
        plt.plot(pred_time_indices, np.abs(error2), 'm:', label='|Prediction 2 Error|', alpha=0.7)
