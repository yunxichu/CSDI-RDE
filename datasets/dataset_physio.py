...
class Physio_Dataset(Dataset):
    def __init__(self, eval_length=48, use_index_list=None, missing_ratio=0.0, seed=0):
        ...
        self.observed_masks = []
        self.gt_masks = []
        self.gt_values = []  # 存储原始值作为ground truth
        path = (
            "./data/physio_missing" + str(missing_ratio) + "_seed" + str(seed) + ".pk"
        )
        meanstd_path = (
            "./data/physio_missing" + str(missing_ratio) + "_seed" + str(seed) + "_meanstd.pk"
        )
        ...
        if os.path.isfile(path) == False:  # if datasetfile is none, create
            ...
            for id_ in idlist:
                try:
                    ...
                    self.gt_values.append(observed_values.copy())  # 保存原始值
            ...
            self.gt_values = np.array(self.gt_values)  # 原始值
            ...
            # 标准化observed_values（用于模型训练）
            self.observed_values = (
                (self.observed_values - mean) / std * self.observed_masks
            )
            # gt_values保持原始值（不标准化），用于评估

            with open(path, "wb") as f:
                pickle.dump(
                    [self.observed_values, self.observed_masks, self.gt_masks, self.gt_values], f
                )
            # 保存mean和std到单独文件
            ...
            with open(path, "rb") as f:
                ...
                if len(loaded_data) == 4:
                    self.observed_values, self.observed_masks, self.gt_masks, self.gt_values = loaded_data
                else:
                    # 旧格式，没有gt_values
                    self.observed_values, self.observed_masks, self.gt_masks = loaded_data
                    self.gt_values = self.observed_values.copy()  # 使用observed_values作为默认值
            # 加载mean和std
            if os.path.isfile(meanstd_path):
                with open(meanstd_path, "rb") as f:
                    self.train_mean, self.train_std = pickle.load(f)
            else:
                # 如果没有保存的mean/std，重新计算
                tmp_values = self.observed_values.reshape(-1, 35)
                tmp_masks = self.observed_masks.reshape(-1, 35)
                mean = np.zeros(35)
                std = np.zeros(35)
    ...
    def __getitem__(self, org_index):
        ...
        s = {
            ...
            "gt_data": self.gt_values[index],  # 添加gt_data字段
...
def get_dataloader(seed=1, nfold=None, batch_size=16, missing_ratio=0.1, device='cpu'):
