    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                if self.is_unconditional == True:
                    diff_input = cond_mask * noisy_cond_history[t] + (1 - cond_mask) * current_sample
...
class CSDI_Lorenz96(CSDI_base):
    def __init__(self, config, device, target_dim=100):
        super(CSDI_Lorenz96, self).__init__(target_dim, config, device)
