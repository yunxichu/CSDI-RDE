...
注意：RDE-GPR 对 36维 + 长horizon 非常慢。建议先用 --target_indices 0,1,2 和小 s 验证流程。
"""
...
def csdi_impute_chunk_pm25(
    ...
    # timepoints: (B,L)；训练时每个样本都是 0..eval_len-1，所以这里 chunk 内也用 0..L-1
    observed_tp = torch.arange(L, device=device, dtype=torch.float32).unsqueeze(0)  # (1,L)

    # 固定采样随机性（impute 内部只用 torch.randn_like）
...
def csdi_impute_history_long(
    ...
    - stride=chunk_len 表示不重叠；你也可以设小点做重叠（这里先用最稳定的实现）
    """
...
# =============================================================================
# 3) RDE-GPR（与你 test2.py 一致，整理为可复用）
# =============================================================================
...
def main():
    ...
    if target_indices is None:
        print("\n[警告] 你选择预测全部 36 维 + horizon 很长时会非常慢。建议先用 --target_indices 0,1,2 测试流程。")

    print("\n开始RDE-GPR滚动预测（后半段，不看未来值）...")
    ...
    metrics = compute_metrics(y_true, preds.astype(np.float64))
