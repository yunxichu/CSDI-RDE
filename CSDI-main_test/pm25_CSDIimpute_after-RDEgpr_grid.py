...
PM2.5 后续预测脚本（不再跑CSDI）- Debug增强版 + Grid Search
- 输入：history_imputed.csv（前半段历史已补值完整）
- 输出：
    - 单次运行：future_pred.csv / future_pred_std.csv / metrics / plots / debug_report.json
    - 网格搜索：grid_search/ 下为每组(L,trainlength)子目录 + grid_summary.csv + heatmap

示例（单次预测1天）：
python pm25_CSDIimpute_after-RDEgpr_grid.py \
  ...
  --split_ratio 0.5 \
  --horizon_days 1 \
  --L 6 --s 50 --trainlength 400 --n_jobs 8 \
...
python pm25_CSDIimpute_after-RDEgpr_grid.py \
  ...
  --split_ratio 0.5 \
  --horizon_days 1 \
  --s 50 --steps_ahead 1 --n_jobs 8 \
  --grid_search

注意：
- 预测阶段不会读取 fut_full 的数值，仅使用其 datetime 索引。
- 若你没有未来真值，不要评估：加 --skip_metrics（但 grid_search 需要评估指标，默认会忽略 skip_metrics）。
"""
...
def sample_random_combinations(D: int, L: int, s: int, rng: np.random.Generator, max_tries: int = None):
    """
    从 {0..D-1} 中随机采样至多 s 个不重复的 L-组合，避免 enumerate 全部组合导致爆内存。
    返回：List[Tuple[int,...]]
    """
    D = int(D)
    L = int(L)
    s = int(s)
    if L > D or L <= 0 or s <= 0:
        return []
    if max_tries is None:
        max_tries = max(1000, s * 50)
    selected = set()
    tries = 0
    while len(selected) < s and tries < max_tries:
        comb = tuple(sorted(rng.choice(D, size=L, replace=False).tolist()))
        selected.add(comb)
        tries += 1
...
def rdegpr_predict_next_for_target(traindata, target_idx, L, s, steps_ahead, pool, rng, optimize_hyp=True, debug=False):
    ...
    - NOTE: 这里改成“随机采样组合”，不再 list(itertools.combinations)，避免 L 大时组合爆炸。
    """
    D = traindata.shape[1]

    selected = sample_random_combinations(D=D, L=int(L), s=int(s), rng=rng)
    if len(selected) == 0:
        dbg = None
        if debug:
            dbg = {
                ...
                "fail_degenerate_count": 0,
                "valid_pred_count": 0,
                "pred_min": None,
                "pred_max": None,
                "pred_mean": None,
                "note": "no valid combinations sampled (L>D or s<=0)"
            }
...
def rdegpr_forecast_multivariate(
    ...
    - 每步用最近 trainlength
    - 未预测维度默认持久性
    - pred_j 若 NaN -> 持久性回退（避免 NaN->0 的隐式污染）
    """
    ...
    try:
        ...
        for step in tqdm(range(horizon), desc="RDE-GPR forecasting", leave=False):
            ...
            if debug and step < debug_steps:
                step_dbg = {
                    ...
                    "baseline_lastrow_stats": basic_array_stats(seq[-1], "baseline_last_row"),
                    "targets": []
                }
...
def save_grid_heatmap(out_dir: str, results_df: pd.DataFrame, metric: str = "rmse"):
    """
    根据 grid_summary.csv 生成 (trainlength x L) 的 heatmap。
    results_df 必须包含列：L, trainlength, metric
    """
    ensure_dir(out_dir)
    if len(results_df) == 0:
        return

    pivot = results_df.pivot(index="trainlength", columns="L", values=metric)
    ...
    plt.xlabel("L")
    plt.ylabel("trainlength")
    plt.title(f"Grid Search Heatmap ({metric.upper()})")
    plt.colorbar(label=metric.upper())
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"grid_heatmap_{metric}.png"), dpi=150)
    plt.close()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="PM2.5 Forecasting: RDE-GPR from Imputed History (Debug Enhanced + Grid Search)")
    ...
    # Grid Search
    parser.add_argument("--grid_search", action="store_true", help="对 L 与 trainlength 做网格搜索")
    parser.add_argument("--grid_L_start", type=int, default=5)
    parser.add_argument("--grid_L_end", type=int, default=20)
    parser.add_argument("--grid_L_step", type=int, default=3)
    parser.add_argument("--grid_train_start", type=int, default=200)
    parser.add_argument("--grid_train_end", type=int, default=2000)
    parser.add_argument("--grid_train_step", type=int, default=200)
    ...
    # 强制检查：NaN/Inf
    base_report["checks"]["history_has_nan"] = bool(np.isnan(history).any())
    base_report["checks"]["history_has_inf"] = bool(np.isinf(history).any())
    assert_or_raise(not base_report["checks"]["history_has_nan"], "history_imputed 内仍包含 NaN：请先确保补值完全。")
    assert_or_raise(not base_report["checks"]["history_has_inf"], "history_imputed 内包含 Inf：数据异常。")

    # 常见“全0导致未来全0”检查
    base_report["checks"]["history_last_row_all_zero"] = bool(np.allclose(history[-1], 0.0))
    if base_report["checks"]["history_last_row_all_zero"]:
        base_report["checks"]["warning"] = (
    ...
    if args.horizon_days and args.horizon_days > 0:
        ...
        base_report["checks"]["steps_per_day_inferred"] = int(steps_per_day)
    ...
    base_report["checks"]["final_horizon_steps"] = int(horizon)
    ...
    base_report["checks"]["target_indices"] = ("ALL" if target_indices is None else target_indices)

    # 参数合法性检查（写入 report，便于定位）
    D = history.shape[1]
    base_report["checks"]["D"] = int(D)
    ...
    # =========================
    if args.grid_search:
        # grid search 一般都需要指标；如果用户显式 skip_metrics，则这里做提示并忽略
        if args.skip_metrics:
            print("[GridSearch] 已开启 --grid_search，但你同时传了 --skip_metrics。网格搜索需要评估指标，这里将忽略 --skip_metrics。")
        args.skip_metrics = False

        grid_dir = os.path.join(out_dir, "grid_search")
        ensure_dir(grid_dir)

        L_values = list(range(int(args.grid_L_start), int(args.grid_L_end) + 1, int(args.grid_L_step)))
        train_values = list(range(int(args.grid_train_start), int(args.grid_train_end) + 1, int(args.grid_train_step)))

        safe_json_dump(
            {
                "L_values": L_values,
                "trainlength_values": train_values,
                "metric": "rmse (lower is better)"
            },
            os.path.join(grid_dir, "grid_spec.json"),
        )
        ...
        results = []
        total_runs = len(L_values) * len(train_values)
        pbar = tqdm(total=total_runs, desc="Grid search (L x trainlength)")

        for L in L_values:
            for trainlength in train_values:
                run_name = f"L{int(L)}_train{int(trainlength)}"
                run_dir = os.path.join(grid_dir, run_name)
                ensure_dir(run_dir)
                ...
                # 快速过滤非法组合
                if int(L) > D:
                    rec["status"] = "SKIP_L_GT_D"
                    results.append(rec)
                    pbar.update(1)
                    continue
                if int(trainlength) > history.shape[0]:
                    rec["status"] = "SKIP_TRAIN_GT_HIST"
                    results.append(rec)
                    pbar.update(1)
                    continue
                ...
                run_args["L"] = int(L)
                run_args["trainlength"] = int(trainlength)
                safe_json_dump(run_args, os.path.join(run_dir, "args.json"))

                # 预测
                try:
                    debug_out = os.path.join(run_dir, "debug_steps") if args.debug else None
                    start_ts = time.time()
                    ...
                    rec["runtime_sec"] = float(time.time() - start_ts)

                    # 保存预测
                    df_pred = pd.DataFrame(preds, index=fut_index, columns=df_full.columns)
                    df_std = pd.DataFrame(stds, index=fut_index, columns=df_full.columns)

                    df_pred.to_csv(os.path.join(run_dir, "future_pred.csv"))
                    df_std.to_csv(os.path.join(run_dir, "future_pred_std.csv"))
                    np.save(os.path.join(run_dir, "future_pred.npy"), preds)
                    np.save(os.path.join(run_dir, "future_pred_std.npy"), stds)

                    # 指标
                    overall = compute_metrics(y_true, preds.astype(np.float64))
                    rec["rmse"] = overall["rmse"]
                    rec["mae"] = overall["mae"]
                    rec["status"] = "OK"
                    ...
                    pd.DataFrame(per_dim).to_csv(os.path.join(run_dir, "metrics_per_dim.csv"), index=False)

                    # 可视化
                    save_plots(run_dir, fut_index, y_true, preds, args.plot_dim)

                    # 轻量 debug_report（保留输入检查 + 本次预测统计）
                    run_report = dict(base_report)
                    run_report["checks"] = dict(base_report.get("checks", {}))
                    run_report["checks"].update({
                        "L": int(L),
                        "trainlength": int(trainlength),
                        "pred_all_zero": bool(np.allclose(preds, 0.0)),
                        "pred_has_nan": bool(np.isnan(preds).any()),
                    })
                    run_report["pred_stats"] = basic_array_stats(preds, "preds")
                    safe_json_dump(run_report, os.path.join(run_dir, "debug_report.json"))

                except Exception as e:
                    rec["status"] = "FAIL_EXCEPTION"
                    rec["error"] = repr(e)
                    with open(os.path.join(run_dir, "error_traceback.txt"), "w") as f:
        ...
        df_res.to_csv(os.path.join(grid_dir, "grid_summary.csv"), index=False)

        # best by rmse
        df_ok = df_res[df_res["status"] == "OK"].copy()
        if len(df_ok) > 0 and df_ok["rmse"].notna().any():
            best_row = df_ok.loc[df_ok["rmse"].idxmin()].to_dict()
            safe_json_dump(best_row, os.path.join(grid_dir, "best_by_rmse.json"))

            # heatmaps
            save_grid_heatmap(grid_dir, df_ok, metric="rmse")
            save_grid_heatmap(grid_dir, df_ok, metric="mae")

        # 保存基础报告（输入数据检查）
        safe_json_dump(base_report, os.path.join(out_dir, "debug_report_data_only.json"))

        print("\nGrid search finished.")
        print("Summary CSV:", os.path.join(grid_dir, "grid_summary.csv"))
        if os.path.exists(os.path.join(grid_dir, "best_by_rmse.json")):
            print("Best params:", os.path.join(grid_dir, "best_by_rmse.json"))
        print("Heatmaps:", os.path.join(grid_dir, "grid_heatmap_rmse.png"), os.path.join(grid_dir, "grid_heatmap_mae.png"))
        print("输出目录：", out_dir)
        return
    ...
    # =========================
    base_report["checks"]["L"] = int(args.L)
    base_report["checks"]["s"] = int(args.s)
    base_report["checks"]["trainlength"] = int(args.trainlength)
    ...
    print("target_indices =", base_report["checks"]["target_indices"])
    ...
    # 预测输出自检（0/NaN）
    base_report["pred_stats"] = basic_array_stats(preds, "preds")
    base_report["pred_first_row_stats"] = basic_array_stats(preds[0], "preds_first_row")
    base_report["pred_last_row_stats"] = basic_array_stats(preds[-1], "preds_last_row")
    base_report["checks"]["pred_all_zero"] = bool(np.allclose(preds, 0.0))
    base_report["checks"]["pred_has_nan"] = bool(np.isnan(preds).any())
    ...
    # debug report
    safe_json_dump(base_report, os.path.join(out_dir, "debug_report.json"))
    ...
    # 最后输出一些高价值诊断提示
    if base_report["checks"]["pred_all_zero"]:
        print("\n[诊断提示] preds 全为0：")
    ...
    if base_report["checks"]["pred_has_nan"]:
